...
def html_data_crafter(tab, meaning):
    if meaning == 0:
        tab = f'<th style="border-radius: 5px 0 0 0;" class="table-lesson-numbers"><div class="table-lesson-num">{tab}</div></th>'
    elif meaning == 7:
        tab = f'<th style="border-radius: 0 5px 0 0;" class="table-lesson-numbers"><div class="table-lesson-num">{tab}</div></th>'
    else:
...
def html_lesson_crafter(tab):
    ...
    if 'Физическая культура и спорт' in tab:
        ...
        tab = tab.replace("(п/г 1)", " ")
        tab = tab.replace("(п/г 2)", " ")
        tab = tab.replace("фв Общая физическая подготовка", " ")
        teacher_name_1 = tab.split()[0] + " " + tab.split()[1]
        teacher_name_2 = tab.split()[-3] + " " + tab.split()[-2]
        cabinet_number_1 = tab.split()[2]
        cabinet_number_2 = tab.split()[-1]
        ...
        if 'Основы информационной культуры и безопасности' in tab:
            ...
            tab = f'<td class="table-subgroups"><div class="table-subgroup-item color-together"><div class="table-sg-name">Подгруппы №1 №2</div><div class="table-lessontype">{type_of_lesson}</div><div class="table-subject">{lesson_name}</div><div class="table-teacher">(№1) {teacher_name_1}</div><div class="table-room">(№2) {teacher_name_2}</div></td>'
        else:
            ...
            tab = f'<td class="table-subgroups"><div class="table-subgroup-item color-together"><div class="table-sg-name">Подгруппы №1 №2</div><div class="table-lessontype">{type_of_lesson}</div><div class="table-subject">{lesson_name}</div></div></td>'
        ...
        if len(re.sub(r'([А-Я])', r' \1', test_tab).split()) >= 2:
            ...
            lesson_name = ' '.join(tab.split()[0:-3])
        tab = f'<td class="table-subgroups"><div class="table-subgroup-item {style}"><div class="table-sg-name">подгруппа №1</div><div class="table-subject">{lesson_name}</div><div class="table-lessontype">{type_of_lesson}</div><div class="table-teacher">{teacher_name}</div><div class="table-room">{cabinet_number}</div></div></td>'
        ...
        if len(re.sub(r'([А-Я])', r' \1', test_tab).split()) >= 2:
            ...
            teacher_name = tab.split()[-3] + " " + tab.split()[-2]
            lesson_name = ' '.join(tab.split()[0:-3])
        tab = f'<td class="table-subgroups"><div class="table-subgroup-item {style}"><div class="table-sg-name">подгруппа №2</div><div class="table-subject">{lesson_name}</div><div class="table-lessontype">{type_of_lesson}</div><div class="table-teacher">{teacher_name}</div><div class="table-room">{cabinet_number}</div></div></td>'
    else:
        test_tab = tab.split()[-3]
        if len(re.sub(r'([А-Я])', r' \1', test_tab).split()) >= 2:
            teacher_name = re.sub(r'([А-Я])', r' \1', test_tab).split()[1] + " " + tab.split()[-2]
        ...
        for article in articles_card:
            try:
                ...
                for table in tables:
                    ...
                    if dd == countt:
                        print(group_name)
                        html2 = '<table id="schedule-table"><tbody><tr>' + html + ' </tr></tbody></table>'
                ...
                full_tables_html = full_tables_html + f'<div class="updatetext" style="text-align: center;">Последнее обновление: {now.strftime("%d-%m-%Y %H:%M:%S")}</div>'
